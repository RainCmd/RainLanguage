...
void FileSpace::Link(DeclarationManager* manager, List<List<AbstractSpace*, true>*, true>* relySpaceCollector)
{
	...
	for(uint32 x = 0; x < classes.Count(); x++)
	{
		...
		for(uint32 y = 0; y < file->inherits.Count(); y++)
		{
			...
			{
				if(findType.dimension) { MESSAGE2(manager->messages, file->inherits[y].name, MessageType::ERROR_NONINHERITABLE_ARRAY); }
				else if(findType.code == TypeCode::Interface)
				{
				...
				else MESSAGE2(manager->messages, file->inherits[y].name, MessageType::ERROR_NOT_INTERFACE_TYPE);
			}
		}
