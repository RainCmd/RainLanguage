...
RainProduct* Build(const BuildParameter& parameter)
{
	...
	DeclarationManager manager = DeclarationManager(parameter.libraryLoader, parameter.libraryUnloader, &stringAgency, messages, stringAgency.Add(parameter.name.value, parameter.name.length));
	PointerList<List<AbstractSpace*, true>> relySpaceCollector(0);

	PointerList<FileSpace> fileSpaces = PointerList<FileSpace>(0);
	while(parameter.codeLoader->LoadNext())
	{
		...
		fileSpaces.Add(new FileSpace(&manager.compilingLibrary, INVALID, &parseParameter));
	}
