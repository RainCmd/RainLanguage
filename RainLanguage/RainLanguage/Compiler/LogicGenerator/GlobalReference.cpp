...
#define DELETE_LIST(list) for (uint32 i = 0; i < list.Count(); i++) delete list[i]; list.Clear();

GlobalReferenceEnum::~GlobalReferenceEnum()
{
	DELETE_LIST(elements);
}

GlobalReferenceStruct::~GlobalReferenceStruct()
{
	DELETE_LIST(variables);
	DELETE_LIST(functions);
}

GlobalReferenceClass::~GlobalReferenceClass()
{
	DELETE_LIST(variables);
	DELETE_LIST(constructors);
	DELETE_LIST(functions);
}

GlobalReferenceInterface::~GlobalReferenceInterface()
{
	DELETE_LIST(functions);
}

GlobalReferenceLibrary::~GlobalReferenceLibrary()
{
	DELETE_LIST(variables);
	DELETE_LIST(enums);
	DELETE_LIST(structs);
	DELETE_LIST(classes);
	DELETE_LIST(interfaces);
	DELETE_LIST(delegates);
	DELETE_LIST(tasks);
	DELETE_LIST(functions);
	DELETE_LIST(natives);
}
...
template<typename T>
uint32 GetGlobalReferenceListIndex(uint32 index, List<T*, true>& list)
{
	for (uint32 i = 0; i < list.Count(); i++)
		if (list[i]->index == index)
			return i;
	list.Add(new T(index));
	return list.Count() - 1;
}

template<typename T>
uint32 GetGlobalReferenceDefinitionIndex(Dictionary<CompilingDeclaration, CompilingDeclaration, true>* declarationMap, const CompilingDeclaration& declaration, uint32 library, uint32 definition, List<T*, true>& list)
{
...
CompilingDeclaration GlobalReference::AddReference(const CompilingDeclaration& declaration)
{
	...
	switch (declaration.category)
	{
		...
		case DeclarationCategory::Struct:
		{
			...
			{
				globalReferenceStruct->variables.Add(new GlobalReferenceStruct::Variable(i));
