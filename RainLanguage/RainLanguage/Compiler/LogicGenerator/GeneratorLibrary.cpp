...
void CollectSpace(AbstractSpace* index, List<Space>& spaces, StringAgency* agency)
{
	...
	uint32 current = spaces.Count() - 1;
	while(spaceIterator.Next())
	{
		spaces[current].children.Add(spaces.Count());
...
{
	Library* result = new Library(new StringAgency(0xf), data, manager.compilingLibrary.dataSize, lambdaStartFunctionIndex, manager.selfLibaray->variables.Count(), manager.selfLibaray->enums.Count(), manager.selfLibaray->structs.Count(), manager.selfLibaray->classes.Count(), manager.selfLibaray->interfaces.Count(), manager.selfLibaray->delegates.Count(), manager.selfLibaray->tasks.Count(), manager.selfLibaray->functions.Count(), manager.selfLibaray->natives.Count(), codeStrings.Count(), dataStrings.Count(), globalReference->libraryReferences.Count(), globalReference->libraries.Count());
	result->code = code;
