...
void ComplexStringExpression::Generator(LogicGenerateParameter& parameter)
{
	...
	for(uint32 i = 0; i < elements.Count(); i++)
	{
			...
			)
		{
			...
			element = new InvokerMemberExpression(element->anchor, toStringFunction->returns.GetTypes(), GetEmptyTupleExpression(), element, toStringFunction->declaration, false);
		}
		else if(!elementType.dimension && elementType.code == TypeCode::Enum)
		{
			...
			element = new InvokerMemberExpression(element->anchor, enumToString->returns.GetTypes(), GetEmptyTupleExpression(), element, enumToString->declaration, false);
		}
		else if(elementType == TYPE_Type)
		{
			...
			element = new InvokerMemberExpression(element->anchor, getNameFunction->returns.GetTypes(), GetEmptyTupleExpression(), element, getNameFunction->declaration, false);
		}
		else if(elementType == TYPE_Entity)
		{
			...
			element = new InvokerMemberExpression(element->anchor, getEntityIDFunction->returns.GetTypes(), GetEmptyTupleExpression(), element, getEntityIDFunction->declaration, false);
		...
		else if(parameter.manager->IsInherit(TYPE_Handle, elementType))
		{
			...
			element = new InvokerVirtualMemberExpression(element->anchor, toStringFunction->returns.GetTypes(), GetEmptyTupleExpression(), element, toStringFunction->declaration, false);
		}
		else if(elementType != TYPE_String)
		{
			...
			element = new InvokerVirtualMemberExpression(element->anchor, getTypeFunction->returns.GetTypes(), GetEmptyTupleExpression(), element, getTypeFunction->declaration, false);
