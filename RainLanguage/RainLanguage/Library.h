...
public:
	...
	inline Library(StringAgency* stringAgency, const List<uint8, true>& constData, uint32 dataSize, uint32 lambdaStart, uint32 variableCount, uint32 enumCount, uint32 structCount, uint32 classCount, uint32 interfaceCount, uint32 delegateCount, uint32 taskCount, uint32 functionCount, uint32 nativeCount, uint32 codeStringCount, uint32 dataStringCount, uint32 libraryReferenceCount, uint32 importCount)
		: stringAgency(stringAgency), spaces(0), code(0), constData(constData), dataSize(dataSize), lambdaStart(lambdaStart), variables(variableCount), enums(enumCount), structs(structCount), classes(classCount), interfaces(interfaceCount), delegates(delegateCount), tasks(taskCount), functions(functionCount), natives(nativeCount), codeStrings(codeStringCount), dataStrings(dataStringCount), libraryReferences(libraryReferenceCount), imports(importCount)
	{
	}
