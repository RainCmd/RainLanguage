...
public:
	...
	inline Space(string name, const List<string, true>& attributes, const List<uint32, true>& children, const List<uint32, true>& variables, const List<uint32, true>& enums, const List<uint32, true>& structs, const List<uint32, true>& classes, const List<uint32, true>& interfaces, const List<uint32, true>& delegates, const List<uint32, true>& coroutines, const List<uint32, true>& functions, const List<uint32, true>& natives)
		:name(name), attributes(attributes), children(children), variables(variables), enums(enums), structs(structs), classes(classes), interfaces(interfaces), delegates(delegates), coroutines(coroutines), functions(functions), natives(natives)
	{}
	inline Space(string name, uint32 attributeCount, uint32 childCount, uint32 variableCount, uint32 enumCount, uint32 structCount, uint32 classCount, uint32 interfaceCount, uint32 delegateCount, uint32 coroutineCount, uint32 functionCount, uint32 nativeCount)
		: name(name), attributes(attributeCount), children(childCount), variables(variableCount), enums(enumCount), structs(structCount), classes(classCount), interfaces(interfaceCount), delegates(delegateCount), coroutines(coroutineCount), functions(functionCount), natives(nativeCount)
	{}
};
...
public:
	...
	List<CoroutineDeclarationInfo> coroutines;
