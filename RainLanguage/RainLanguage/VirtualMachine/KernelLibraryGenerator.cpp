...
static StringAgency* stringAgency = new StringAgency(1024);
#define EMPTY_STRINGS (List<string, true>(0))
uint32 AddSpace(List<Space>& spaces, KernelLibraryInfo::Space* kernelSpace)
{
	...
	for (uint32 i = 0; i < kernelSpace->children.Count(); i++)
		spaces[index].children.Add(AddSpace(spaces, kernelSpace->children[i]));
	return index;
}
...
Library* GetKernelLibrary()
{
	if (!kernelLibrary)
	{
		...
		kernelLibrary = new Library(stringAgency, library->data, library->data.Count(), library->variables.Count(), library->enums.Count(), library->structs.Count(), library->classes.Count(), library->interfaces.Count(), library->delegates.Count(), library->coroutines.Count(), library->functions.Count(), 0, 0, 0, 0, 0);
		AddSpace(kernelLibrary->spaces, library->root);
		...
		for (uint32 x = 0; x < library->enums.Count(); x++)
		{
			...
			for (uint32 y = 0; y < source->elements.Count(); y++)
				new (elements.Add())EnumDeclarationInfo::Element(stringAgency->AddAndRef(source->elements[y].name), source->elements[y].value);
			new (kernelLibrary->enums.Add())EnumDeclarationInfo(true, EMPTY_STRINGS, stringAgency->AddAndRef(source->name), elements);
		}
		...
		for (uint32 x = 0; x < library->structs.Count(); x++)
		{
			...
			for (uint32 y = 0; y < source->variables.Count(); y++)
			{
				...
				new (memberVariables.Add())VariableDeclarationInfo(false, EMPTY_STRINGS, stringAgency->AddAndRef(variable->name), variable->type, variable->address, true);
			}
			new (kernelLibrary->structs.Add())StructDeclarationInfo(true, EMPTY_STRINGS, stringAgency->AddAndRef(source->name), memberVariables, source->functions, source->size, source->alignment);
		}
		...
		for (uint32 x = 0; x < library->classes.Count(); x++)
		{
			...
			{
				const KernelLibraryInfo::Variable* variable = &source->variables[y];
				new (memberVariables.Add())ReferenceVariableDeclarationInfo(false, EMPTY_STRINGS, stringAgency->AddAndRef(variable->name), variable->type, variable->address, true, List<VariableReference, true>(0));
			}
			...
			new (kernelLibrary->classes.Add())ClassDeclarationInfo(true, EMPTY_STRINGS, stringAgency->AddAndRef(source->name), source->parent, source->inherits, source->size, source->alignment, List<uint32, true>(0), memberVariables, source->functions, INVALID, relocations);
		}
		...
		for (uint32 x = 0; x < library->interfaces.Count(); x++)
		{
			...
			for (uint32 y = 0; y < source->functions.Count(); y++)
			{
				...
				new (functions.Add())InterfaceDeclarationInfo::FunctionInfo(EMPTY_STRINGS, stringAgency->AddAndRef(function.name), function.returns, function.parameters);

				MemberFunction realize = MemberFunction(LIBRARY_KERNEL, TypeCode::Interface, x, y);
				Set<Declaration, true>::Iterator iterator = allInherits.GetIterator();
				while (iterator.Next()) CollectRelocation(library, relocations, realize, function.name, function, iterator.Current());
			}
			new (kernelLibrary->interfaces.Add())InterfaceDeclarationInfo(true, EMPTY_STRINGS, stringAgency->AddAndRef(source->name), source->inherits, functions, relocations);
		}

		for (uint32 i = 0; i < library->delegates.Count(); i++)
		{
			const KernelLibraryInfo::Delegate* source = &library->delegates[i];
			new (kernelLibrary->delegates.Add())DelegateDeclarationInfo(true, EMPTY_STRINGS, stringAgency->AddAndRef(source->name), source->returns, source->parameters);
		}

		for (uint32 i = 0; i < library->coroutines.Count(); i++)
		{
			const KernelLibraryInfo::Coroutine* source = &library->coroutines[i];
			new (kernelLibrary->coroutines.Add())CoroutineDeclarationInfo(true, EMPTY_STRINGS, stringAgency->AddAndRef(source->name), source->returns);
		}
